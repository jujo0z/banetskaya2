    ...
    <div className="p-6 lg:p-8 space-y-6" data-testid="forma19-page">
      <div>
        <h1 className="font-heading text-3xl font-bold tracking-tight flex items-center gap-3">
          <ClipboardList className="h-8 w-8 text-[#EC4899]" /> Адресный листок (Форма 19)
        </h1>
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-pink-100 bg-white/80 p-4 space-y-2">
            <div className="text-sm font-semibold flex items-center gap-2">
              <Upload className="h-4 w-4 text-[#EC4899]" /> Загрузка студентов из Excel
            </div>
            <input
              ref={fileRef}
              type="file"
            ...
            <Button
              ...
              variant="ghost"
              className="w-full text-[#EC4899] hover:text-[#EC4899]"
          ...
          <div className="rounded-lg border border-[#EC4899]/30 bg-gradient-to-br from-[#EC4899]/10 to-transparent p-4 space-y-2" data-testid="f19-package-box">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <ClipboardList className="h-4 w-4 text-[#EC4899]" /> Полный пакет документов
            </div>
            <p className="text-[11px] text-muted-foreground">
            ...
            <div className="flex gap-2">
              <Button
                ...
                className="flex-1 bg-[#EC4899] hover:bg-[#DB2777] text-white"
          ...
          <div className="rounded-lg border border-[#EC4899]/30 bg-[#EC4899]/5 p-4 space-y-2" data-testid="f19-defaults-box">
            <button
              type="button"
            ...
            >
              <span className="flex items-center gap-2">
                <Wand2 className="h-4 w-4 text-[#EC4899]" /> Постоянные значения
              </span>
            ...
            {showDefaults && (
              <div className="space-y-3 pt-2 max-h-[420px] overflow-auto pr-1">
                {groups.map((g) => (
                  <div key={g.group}>
                    <div className="text-[10px] uppercase tracking-wider text-[#EC4899] font-semibold mb-1.5">
                      {g.group}
                    </div>
          ...
          <div className="rounded-lg border border-pink-100 bg-white/80 p-4 space-y-3">
            <div className="text-sm font-semibold flex items-center gap-2">
              <Printer className="h-4 w-4 text-[#EC4899]" /> Двусторонняя печать
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`rounded-md border px-3 py-2 text-sm text-left transition ${
                  duplexFlip === "long" ? "border-[#EC4899] bg-[#EC4899]/10" : "border-pink-100 hover:border-pink-200"
                }`}
              ...
              <button
                ...
                className={`rounded-md border px-3 py-2 text-sm text-left transition ${
                  duplexFlip === "short" ? "border-[#EC4899] bg-[#EC4899]/10" : "border-pink-100 hover:border-pink-200"
                }`}
            ...
            <div className="text-[12px] text-muted-foreground bg-white/80 rounded-md px-3 py-2">
              Людей: <b className="text-slate-800">{records.length}</b> · листов A4 (лицо+оборот):{" "}
              <b className="text-slate-800">{sheets}</b>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <Button onClick={doOpen} className="bg-[#EC4899] hover:bg-[#DB2777]" data-testid="f19-open">
          ...
          <div className="rounded-lg border border-pink-100 bg-white/80 p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-[520px] overflow-auto pr-1">
              {records.map((r, idx) => {
                ...
                return (
                  <div key={idx} className="rounded-md border border-pink-100 bg-white/60" data-testid={`f19-record-${idx}`}>
                    <div className="flex items-center gap-1 px-2 py-1.5">
                      ...
                      <button type="button" title="Дублировать" onClick={() => duplicateRecord(idx)} className="p-1.5 text-muted-foreground hover:text-slate-900" data-testid={`f19-record-dup-${idx}`}>
                      ...
                      <button type="button" title="Удалить" onClick={() => removeRecord(idx)} className="p-1.5 text-muted-foreground hover:text-[#EC4899]" data-testid={`f19-record-del-${idx}`}>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold text-muted-foreground">
              {editMode ? "Редактор шаблона Формы 19" : `Предпросмотр листа (${previewSide === "back" ? "оборотная сторона" : "лицевая сторона"})`}
            </div>
