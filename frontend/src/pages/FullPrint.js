    ...
    <div className="p-6 lg:p-8 space-y-6" data-testid="fullprint-page">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[380px_minmax(0,1fr)] gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-4 space-y-3">
            <div className="text-sm font-semibold flex items-center gap-2">
              <Printer className="h-4 w-4 text-[#E11D48]" /> Раскладка на листе
            </div>
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`rounded-md border px-3 py-2 text-sm text-left transition ${
                  orientation === "portrait" ? "border-[#E11D48] bg-[#E11D48]/10" : "border-white/10 hover:border-white/30"
                }`}
              ...
              <button
                ...
                className={`rounded-md border px-3 py-2 text-sm text-left transition ${
                  orientation === "landscape" ? "border-[#E11D48] bg-[#E11D48]/10" : "border-white/10 hover:border-white/30"
                }`}
                ...
                {Array.from({ length: maxPerSheet }, (_, i) => i + 1).map((n) => (
                  <button
                    ...
                    className={`flex-1 rounded-md border px-2 py-1.5 text-sm transition ${
                      perSheet === n ? "border-[#E11D48] bg-[#E11D48]/10 font-semibold" : "border-white/10 hover:border-white/30"
                    }`}
            ...
            <div className="text-[12px] text-muted-foreground bg-white/5 rounded-md px-3 py-2">
              Записей: <b className="text-white">{records.length}</b> · листов A4:{" "}
          ...
          <div className="rounded-lg border border-white/10 bg-white/5 p-4 space-y-3">
            ...
            <div className="space-y-2 max-h-[520px] overflow-auto pr-1">
              {records.map((r, idx) => {
                ...
                return (
                  <div key={idx} className="rounded-md border border-white/10 bg-black/20" data-testid={`record-${idx}`}>
                    <div className="flex items-center gap-1 px-2 py-1.5">
                      ...
                      <button type="button" title="Случайные" onClick={() => fillRandomAt(idx)} className="p-1.5 text-muted-foreground hover:text-white" data-testid={`record-random-${idx}`}>
                        <Shuffle className="h-3.5 w-3.5" />
                      </button>
                      <button type="button" title="Дублировать" onClick={() => duplicateRecord(idx)} className="p-1.5 text-muted-foreground hover:text-white" data-testid={`record-dup-${idx}`}>
                      ...
                      <button type="button" title="Удалить" onClick={() => removeRecord(idx)} className="p-1.5 text-muted-foreground hover:text-[#E11D48]" data-testid={`record-del-${idx}`}>
                    ...
                    {open && (
                      <div className="px-3 pb-3 pt-1 space-y-3 border-t border-white/10">
                        {Object.entries(groups).map(([g, fs]) => (
                          <div key={g}>
                            <div className="text-[10px] uppercase tracking-wider text-[#E11D48] font-semibold mb-1.5">{g}</div>
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-lg border border-white/10 bg-white overflow-hidden shadow-2xl flex items-center justify-center" style={{ height: "78vh" }}>
            {previewUrl ? (
