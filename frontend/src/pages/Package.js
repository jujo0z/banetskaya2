    ...
    <div className="space-y-8" data-testid="package-page">
      ...
      <div>
        <div className="flex items-center gap-3">
          <div className="h-11 w-11 rounded-xl bg-gradient-to-br from-[#F43F5E] to-[#DB2777] flex items-center justify-center glow-crimson">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Step 1 — Excel */}
          <div className="glass rounded-2xl border border-pink-100 p-6 space-y-4">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <span className="h-6 w-6 rounded-full bg-[#EC4899] text-white text-xs flex items-center justify-center">1</span>
          ...
          {/* Loaded people */}
          <div className="glass rounded-2xl border border-pink-100 p-6">
            ...
            {people.length === 0 ? (
              ...
              <div className="flex flex-wrap gap-2" data-testid="package-people-list">
                {people.map((p, i) => (
                  <span
                    key={i}
                    className="px-3 py-1.5 rounded-lg bg-white/80 border border-pink-100 text-sm"
                  >
                    {personLabel(p, i)}
                  </span>
                ))}
        ...
        <div className="space-y-6">
          {/* Step 2 — documents */}
          <div className="glass rounded-2xl border border-pink-100 p-6 space-y-3">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <span className="h-6 w-6 rounded-full bg-[#EC4899] text-white text-xs flex items-center justify-center">2</span>
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-sm transition-all ${
                    include[key]
                      ...
                      : "border-pink-100 text-muted-foreground hover:border-pink-200"
                  }`}
                  data-testid={`package-toggle-${key}`}
                >
                  <span
                    className={`h-4 w-4 rounded flex items-center justify-center text-[10px] ${
                      include[key] ? "bg-[#EC4899] text-white" : "bg-white"
                    }`}
                  >
                    {include[key] ? "✓" : ""}
                  </span>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={`px-3 py-2 rounded-lg border text-xs transition-all ${
                  duplexFlip === "long" ? "border-[#EC4899] bg-[#EC4899]/10 text-white" : "border-pink-100 text-muted-foreground"
                }`}
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg border text-xs transition-all ${
                  duplexFlip === "short" ? "border-[#EC4899] bg-[#EC4899]/10 text-white" : "border-pink-100 text-muted-foreground"
                }`}
          ...
          {/* Step 3 — actions */}
          <div className="glass rounded-2xl border border-[#EC4899]/30 p-6 space-y-3">
            <div className="flex items-center gap-2 text-sm font-semibold">
              <span className="h-6 w-6 rounded-full bg-[#EC4899] text-white text-xs flex items-center justify-center">3</span>
              Печать / сохранение
            </div>
            <Button
              onClick={() => run("open")}
