...
export default function DocumentEditor({
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent
        className="max-w-[1400px] w-[97vw] max-h-[94vh] p-0 gap-0 overflow-hidden rounded-lg bg-card border-pink-100"
        data-testid="document-editor"
      >
        {/* Header */}
        <DialogHeader className="px-6 py-4 border-b border-pink-100 bg-gradient-to-r from-[#EC4899]/10 via-transparent to-transparent">
            ...
            <div className="flex items-center gap-2">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="rounded-sm border-pink-200" data-testid="preset-apply-btn">
              ...
              <Popover open={presetOpen} onOpenChange={setPresetOpen}>
                <PopoverTrigger asChild>
                  <Button variant="outline" size="sm" className="rounded-sm border-pink-200" data-testid="preset-save-btn">
                ...
                <PopoverContent align="end" className="w-72">
                  ...
                  />
                  <Button
                    className="w-full rounded-sm bg-[#EC4899] hover:bg-[#DB2777] text-white"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)] min-h-0">
          {/* Left: editor */}
          <div className="overflow-y-auto max-h-[62vh] lg:max-h-[74vh] px-6 py-5 border-b lg:border-b-0 lg:border-r border-pink-100">
            <Accordion type="multiple" defaultValue={["contract", "tenant", "sections"]} className="space-y-3">
              {FIELD_GROUPS.map((g) => {
                ...
                return (
                  <AccordionItem
                    key={g.id}
                    value={g.id}
                    className="border border-pink-100 rounded-md bg-[#FDF4F9]/60 px-4"
                  >
                    <AccordionTrigger className="hover:no-underline py-3" data-testid={`group-${g.id}`}>
                      <span className="flex items-center gap-2.5 text-base font-heading">
                        <Icon className="h-4 w-4 text-[#EC4899]" /> {g.title}
                      </span>
                    </AccordionTrigger>
                    <AccordionContent>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pb-2">
                        {g.keys.map((key) => (
                          <div key={key} className="space-y-1.5">
                            ...
                            <Input
                              className="rounded-sm bg-[#FDF4F9] border-pink-100 focus:border-[#EC4899] h-9"
                              value={fields[key] || ""}
              ...
              {(() => {
                ...
                return (
                  <div className="border border-pink-100 rounded-md bg-[#FDF4F9]/60 px-4 py-3.5">
                    <label className="flex items-start gap-3 cursor-pointer">
                      <Checkbox
                        checked={checked}
                        onCheckedChange={(v) => update("show_minor_consent", v ? "1" : "0")}
                        className="mt-0.5 border-pink-200 data-[state=checked]:bg-[#EC4899] data-[state=checked]:border-[#EC4899]"
                        data-testid="minor-consent-checkbox"
                      />
                      <span className="space-y-1">
                        <span className="flex items-center gap-2 text-base font-heading">
                          <FileSignature className="h-4 w-4 text-[#EC4899]" />
                          Согласие за несовершеннолетнего
                        </span>
              ...
              <AccordionItem value="sections" className="border border-pink-100 rounded-md bg-[#FDF4F9]/60 px-4">
                <AccordionTrigger className="hover:no-underline py-3" data-testid="group-sections">
                  <span className="flex items-center gap-2.5 text-base font-heading">
                    <SlidersHorizontal className="h-4 w-4 text-[#EC4899]" /> Пункты договора
                  </span>
                </AccordionTrigger>
                <AccordionContent>
                  ...
                  <div className="space-y-4">
                    {SECTIONS.map((s) => (
                      <div key={s.key} className="space-y-1.5">
                        <Label className="text-sm text-slate-800">{s.title}</Label>
                        <Textarea
                          className="rounded-sm bg-[#FDF4F9] border-pink-100 focus:border-[#EC4899] min-h-[70px]"
          ...
          {/* Right: live preview */}
          <div className="flex flex-col bg-[#FDF4F9] max-h-[62vh] lg:max-h-[74vh]">
            <div className="flex items-center justify-between gap-3 px-4 py-2.5 border-b border-pink-100">
        ...
        {/* Footer */}
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 px-6 py-3 border-t border-pink-100 bg-card">
          <Button
            variant="outline"
          ...
          <div className="flex flex-wrap gap-2 justify-end">
            <Button
              variant="outline" className="rounded-sm border-pink-200"
