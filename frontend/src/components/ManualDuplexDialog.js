...
export default function ManualDuplexDialog({ open, ids, onClose }) {
    ...
    return (
      <button type="button" disabled={locked} onClick={() => set(val)}
        className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-sm text-sm border transition-colors ${
          cur === val ? "bg-[#EC4899] text-white border-[#EC4899]" : "border-pink-200 text-muted-foreground hover:text-slate-900"
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="max-w-2xl max-h-[92vh] overflow-y-auto rounded-lg bg-card border-pink-100" data-testid="manual-duplex-dialog">
        <DialogHeader>
          <DialogTitle className="font-heading text-3xl tracking-tight flex items-center gap-2">
            <Layers className="h-6 w-6 text-[#EC4899]" /> Двусторонняя печать вручную
          </DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 mt-2">
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-sm border-pink-200" data-testid="duplex-profiles-btn">
            ...
            <Popover open={saveOpen} onOpenChange={setSaveOpen}>
              <PopoverTrigger asChild>
                <Button variant="outline" size="sm" className="rounded-sm border-pink-200" data-testid="duplex-profile-save-btn">
              ...
              <PopoverContent align="end" className="w-72">
                ...
                <Button className="w-full rounded-sm bg-[#EC4899] hover:bg-[#DB2777] text-white" onClick={handleSaveProfile} disabled={!profileName.trim()} data-testid="duplex-profile-save-confirm">Сохранить</Button>
              </PopoverContent>
            </Popover>
          </div>

          {/* Options */}
          <div className="border border-pink-100 rounded-md p-4 bg-[#FDF4F9]/60 space-y-4">
            <div>
              <div className="flex items-center gap-2 mb-2"><SeparatorHorizontal className="h-4 w-4 text-[#EC4899]" /><span className="font-heading text-base">Диапазон (партия)</span></div>
              <div className="flex items-end gap-3 flex-wrap">
                <div className="space-y-1"><Label className="text-xs text-muted-foreground">Договоры с</Label>
                  <Input type="number" min={1} max={total} disabled={locked} className="w-24 rounded-sm bg-[#FDF4F9] border-pink-100 h-9" value={from} onChange={(e) => setFrom(e.target.value)} data-testid="duplex-range-from" /></div>
                <div className="space-y-1"><Label className="text-xs text-muted-foreground">по</Label>
                  <Input type="number" min={1} max={total} disabled={locked} className="w-24 rounded-sm bg-[#FDF4F9] border-pink-100 h-9" value={to} onChange={(e) => setTo(e.target.value)} data-testid="duplex-range-to" /></div>
                <div className="text-sm text-muted-foreground pb-2">в партии: <b className="text-slate-800">{rangeCount}</b> из {total}</div>
              </div>
              <div className="flex items-center gap-2 mt-2 text-sm text-[#EC4899]" data-testid="duplex-paper-estimate">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2"><FileText className="h-4 w-4 text-[#EC4899]" /><span className="font-heading text-base">Ориентация</span></div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2"><BookOpen className="h-4 w-4 text-[#EC4899]" /><span className="font-heading text-base">Переворот стопки</span></div>
              <div className="flex gap-2">
          ...
          {/* Stepper */}
          {stage === "front" && (
            <div className="border border-[#EC4899]/40 rounded-md p-5 bg-[#EC4899]/5" data-testid="duplex-stage-front">
              <div className="font-heading text-lg flex items-center gap-2 mb-1"><span className="h-6 w-6 rounded-full bg-[#EC4899] text-white text-sm flex items-center justify-center">1</span>Шаг 1. Печать лицевых</div>
              <p className="text-sm text-muted-foreground mb-4">Дождитесь, пока принтер напечатает и остановится.</p>
              <Button className="w-full rounded-sm bg-[#EC4899] hover:bg-[#DB2777] text-white h-11 text-base" onClick={() => printSide("front")} disabled={!!busy || !rangeCount} data-testid="duplex-front-btn">
          ...
          {stage === "flip" && (
            <div className="border border-amber-500/40 rounded-md p-5 bg-amber-500/5" data-testid="duplex-stage-flip">
              ...
              <p className="text-sm text-muted-foreground mb-4">Принтер остановился? Переверните <b className="text-slate-800">всю стопку</b> и вставьте обратно. Затем печатайте обороты.</p>
              <div className="flex flex-col sm:flex-row gap-2">
                <Button className="flex-1 rounded-sm bg-[#EC4899] hover:bg-[#DB2777] text-white h-11 text-base" onClick={() => printSide("back")} disabled={!!busy} data-testid="duplex-back-btn">
                  {busy === "back" ? <Loader2 className="h-5 w-5 animate-spin" /> : <Printer className="h-5 w-5" />} Шаг 2. Печать оборотов ({rangeCount})
                </Button>
                <Button variant="outline" className="rounded-sm border-pink-200" onClick={() => setStage("front")} disabled={!!busy} data-testid="duplex-reprint-front-btn"><RefreshCw className="h-4 w-4" /> Перепечатать лицевые</Button>
              </div>
            </div>
          )}
          {stage === "done" && (
            <div className="border border-emerald-500/40 rounded-md p-5 bg-emerald-500/5" data-testid="duplex-stage-done">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                {t < total ? (
                  <Button className="flex-1 rounded-sm bg-[#EC4899] hover:bg-[#DB2777] text-white" onClick={nextBatch} data-testid="duplex-next-batch-btn"><Printer className="h-4 w-4" /> Следующая партия ({t + 1}–{total})</Button>
                ) : (
                ...
                <Button variant="outline" className="rounded-sm border-pink-200" onClick={() => { setFrom(1); setTo(total); setStage("front"); }} data-testid="duplex-restart-btn"><RotateCcw className="h-4 w-4" /> Начать заново</Button>
              </div>
            </div>
          )}

          {/* Test */}
          <div className="border border-dashed border-pink-200 rounded-md p-4 bg-[#FDF4F9]/40">
            <div className="flex items-center gap-2 mb-1"><FlaskConical className="h-4 w-4 text-[#EC4899]" /><span className="font-heading text-base">Пробный лист (тест переворота)</span></div>
            ...
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" className="rounded-sm border-pink-200" onClick={() => printTest("front")} disabled={!!busy} data-testid="duplex-test-front-btn">
                {busy === "test-front" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Printer className="h-4 w-4" />} Пробная лицевая</Button>
              <Button variant="outline" className="rounded-sm border-pink-200" onClick={() => printTest("back")} disabled={!!busy} data-testid="duplex-test-back-btn">
