...
export default function ZayavPreview({
  ...
  const startDrag = (e, el) => {
    ...
    dragRef.current = { id: el.id, startX: e.clientX, startY: e.clientY, ox: el.x, oy: el.y, rw: rect.width, rh: rect.height };
  ...
  const moveHandle = (el) => (
    <div
      ...
      title="Перетащить"
      data-testid={`zayav-handle-${el.id}`}
      style={{
        position: "absolute", left: `${el.x}%`, top: `${el.y}%`,
        transform: "translate(-16px, -20px)", width: 14, height: 14,
        borderRadius: 3, background: "#E11D48", border: "2px solid #fff",
        boxShadow: "0 1px 4px rgba(0,0,0,.4)", cursor: "move", zIndex: 20,
      }}
    />
  );
  ...
  const renderTextLike = (el) => {
    ...
    const common = {
      position: "absolute", left: `${el.x}%`, top: `${el.y}%`,
      transform: `translate(${tx}, -0.82em)`, transformOrigin: "left top",
      fontFamily: fam, fontSize: `${fontPx}px`, lineHeight: 1,
    ...
    if (editing && !isField) {
      return (
        <div key={el.id} ref={editRef} contentEditable suppressContentEditableWarning
          onBlur={(e) => onCommitText(el.id, e.currentTarget.textContent)}
          onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); e.currentTarget.blur(); } }}
          data-testid={`zayav-el-${el.id}`}
          style={{ ...common, outline: "1.5px solid #2563eb", background: "rgba(37,99,235,.10)", cursor: "text", minWidth: 6, padding: "0 1px" }}>
          {el.text}
        </div>
      );
    }
    ...
    return (
      <div key={el.id}
        onClick={(e) => { if (!editMode) return; e.stopPropagation(); onSelect(el.id); if (!isField) onStartTextEdit(el.id); }}
        title={isField ? `Поле: ${el.field}` : (editMode ? "Клик — редактировать текст" : "")}
        data-testid={`zayav-el-${el.id}`}
        style={{
          ...common,
          cursor: editMode ? (isField ? "pointer" : "text") : "default",
          outline: isSel ? "1.5px solid #E11D48" : (editMode ? "1px dotted rgba(120,120,140,.35)" : "none"),
  ...
  const renderLine = (el) => {
    ...
    return (
      <div key={el.id}
        ...
        data-testid={`zayav-el-${el.id}`} title={editMode ? "Линия" : ""}
        style={{
          position: "absolute", left: `${el.x}%`, top: `${el.y}%`, width: `${el.w}%`,
          height: `${thick}px`, background: el.color || "#17171f",
          cursor: editMode ? "pointer" : "default",
          outline: isSel ? "1.5px solid #E11D48" : "none", outlineOffset: "3px",
        }} />
    );
  };
