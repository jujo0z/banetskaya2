...
def _draw_forma24_back(c, zw, zh, rec):
    ...
    raw = [6, 2.5, 9, 3,          # 12: label / caption / value / gap  -> y1..y4
    ...
    shade(L, y[22], R, B)            # нижние подписи "дата"/"подпись"
    ...
    hline(L, R, y[2])
    cap((L + R) / 2, cb(y[1], y[2]) + 0.2,
        "наименование предприятия, организации, учреждения, должности; если не работает, "
        "то указать пенсионер, учащийся, иждивенец и т.п.", 3.4)
    ...
    lbl(64, r15 + 1.8, "сколько", 5.2)
    cval(90, r15 - 0.8, G(rec, "children_count"), 7.5)
    hline(L, R, y[12])
    ...
    vline(52, y[21], B)
    r17 = cb(y[21], y[22])
    lbl(18, r17, "20", 5.8); hline(22, 34, r17 + 0.7); lbl(35, r17, "г.", 5.8)
    hline(L, R, y[22])
    cap(27, cb(y[22], B) + 0.3, "дата", 4.4); cap(78, cb(y[22], B) + 0.3, "подпись", 4.4)
